...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function showRiskAnalysis(text) {
    // …此处保留你原有的大量风险关键词与提示逻辑（完全一致）…
    // （下行注释：你的原始风险规则与 UI 渲染详见源码） :contentReference[oaicite:5]{index=5}
  }

  // 拖拽上传
  dropZone.addEventListener("dragover", e => { e.preventDefault(); dropZone.style.borderColor = "#005a9e"; });
  dropZone.addEventListener("dragleave", () => { dropZone.style.borderColor = "#999"; });
  dropZone.addEventListener("drop", e => {
    e.preventDefault(); const file = e.dataTransfer.files[0]; if (file) handleFile(file); dropZone.style.borderColor = "#999";
  });
  ...
  togglePreviewBtn.addEventListener("click", () => {
    if (contractText.style.maxHeight === "200px" || contractText.style.maxHeight === "") {
      contractText.style.maxHeight = "none"; togglePreviewBtn.textContent = "收起";
    } else { contractText.style.maxHeight = "200px"; togglePreviewBtn.textContent = "展开"; }
  });
  ...
  sendChatBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      aiBubble.classList.add("ai-response");
      const segments = answer.split(/\n\s*\n|(?=^\s*[①②③\d]+\.)|(?=^\s*[-·•])/gm);
      segments.forEach(seg => { if (seg.trim()) { const p = document.createElement("p"); p.textContent = seg.trim(); aiBubble.appendChild(p); } });
  ...
  function updateCurrentTime() {
    const now = new Date();
    document.getElementById("current-time").textContent = `当前时间：${now.toLocaleString("zh-CN")}`;
  }
